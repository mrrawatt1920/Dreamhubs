...
async function handleAdminPage() {
  ...
  window.refreshAdminDashboard = async function() {
    try {
      ...
      setText("[data-admin-total-balance]", `â‚¹${Number(data.stats.totalBalance || 0).toFixed(2)}`);
      setText("[data-admin-total-spend]", `â‚¹${Number(data.stats.totalSpend || 0).toFixed(2)}`);
